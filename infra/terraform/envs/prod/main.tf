...
variable "zone_id" {
  description = "Cloudflare zone ID for amendment-atoms.com."
  type        = string
}
...
# --- State imports ----------------------------------------------------------
# No existing resources to import for amendment-atoms — the Pages project
# is created on first deploy.yml run. If the project is created out-of-band
# later (wrangler, dashboard, direct API), add an `import` block here so
# terraform adopts the resource without destroy-and-create.
